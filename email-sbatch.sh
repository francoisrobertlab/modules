...
then
  ...
  if grep -Fq "export JOB_MAIL=" ~/.bash_profile
  then
    INDEX=$(grep -n "JOB_MAIL=" ~/.bash_profile | cut -d: -f1)
    sed -i "$((INDEX))d" ~/.bash_profile
  fi
  if grep -Fq "alias sbatch=" ~/.bash_profile
  then
...
if grep -Fq "alias sbatch=" ~/.bash_profile
then
  sed -i -E "s/alias sbatch=.+/alias sbatch='sbatch --mail-type=END,FAIL --mail-user=\$JOB_MAIL'/" ~/.bash_profile
else
  echo "Adding email notification for sbatch"
  echo "alias sbatch='sbatch --mail-type=END,FAIL --mail-user=\$JOB_MAIL'" >> ~/.bash_profile
fi
